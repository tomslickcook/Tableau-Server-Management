...
SELECT
    ...
    CASE
            WHEN s.site_id = 40 THEN 'ED'
            WHEN s.site_id = 20 THEN 'MCIT'
        END
    AS site
FROM
    subscriptions s
    ...
    LEFT OUTER JOIN _views v ON replace(sv.repository_url,'/sheets/','/') = v.view_url --this join is sketchy since we are matching the workbook and view name in theory there could be dups across sites. 
...
WHERE
    (
        s.site_id = 40 --ED Site
        OR (
            s.site_id = 20 --MCIT Site
            AND (
                v.owner_name = 'lemusj' --Where lemus is the view or workbook owner, depending on what is subscribed to
                OR w.owner_name = 'lemusj'
            )
        )
    )